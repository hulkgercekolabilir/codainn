...
module.exports = {
    ...
    execute(message, args) {
        ...
        async () => {
            let panel = await this.db.fetch(`sunucupanel.${message.guild.id}`);
            if (panel)
                return message.channel.send(
                    `Panel Zaten Ayarlanmış Silmek İçin; \`g!panel-sil\``
                );
        };

        let every = message.guild.roles.cache.find(
            (r) => r.name === "@everyone"
        );
        message.guild.channels
            ...
            .then((kategori) => {
                ...
                message.guild.channels
                    .create(`Toplam Üye • ${message.guild.memberCount}`, {
                        type: "voice",
                        reason: "İstatistik",
                    })
                    .then((toplamüye) => {
                            ...
                            .create(
                                `Aktif Üye • ${
                                    message.guild.members.cache.filter(
                                        (off) =>
                                            off.presence.status !== "offline"
                                    ).size
                                }`,
                                { type: "voice", reason: "İstatistik" }
                            )
                            .then((atkifüye) => {
                                message.guild.channels
                                    .create(
